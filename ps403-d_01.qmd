...
How about $\text{poco} \cup \text{eu}$? Use OR (`|`) operator instead `&`.
...
Let's double check. This is basic (and a bit incorrect due to the nature of the data) way of thinking about probability, but quite helpful to start developing the intuition!
...
for(i in 1:100){
  temporary_sample <- sample_n(cpds_subset, size = 80)                   # sample 80 observations
