...
Start with exploring the distribution of tenure for people in cabinet (`average_total`)
...
You can inform you reader by providing extra information. Add the average to the plot using `geom_vline()` and change the number of `bins` to 20. Save the graph to the `avgind_plot` object.
...
Change the breaks to a more readable format to better convey the average. 
...
-   What is the alternative plot more suitable for the comparison of distributions?
...
Let's draw a scatter plot using `geom_point()`. It's more appropriate if we are interested in relationship, rather than distribution comparison. What's going on?
...
leaderexperience_continuous and n_individuals
